...
public fun FlowContent.inventoryIllustration() {
    div(className = "flex justify-center mt-8 opacity-60 transition-opacity duration-500") {
        div(className = "perspective-[1000px]") {
            div(className = "transform rotate-y-[-35deg] rotate-x-[25deg] rotate-z-[15deg] transition-transform duration-500") {
...
private fun FlowContent.inventorySection(size: Int, selectedSlots: List<Int>): Tag {
    return div(className = "grid grid-cols-9 gap-2 p-6 bg-dark-accent/40 rounded-lg border border-light-accent/20 backdrop-blur-sm shadow-2xl shadow-black/50") {
        repeat(size) { index ->
